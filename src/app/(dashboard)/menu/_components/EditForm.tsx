...
import { getAllData, getById, updated } from '@/services/api';
import Form from '@/components/form/Form';
import { toast } from 'react-toastify';
import { EditFormValues, Menu } from '../_interfaces/Menu';
...
const EditForm: React.FC<EditModalsProps<Menu>> = ({
  title,
  initValues,
  isOpen,
  onClose,
  ...
  const schema = yup.object().shape({
    ...
    isActive: yup.boolean().required(t('validate.fieldRequired')),
  });
  ...
  useEffect(() => {
    if (isOpen && initValues) {
      ...
      Promise.all([
        getById('/api/v1/menu', initValues.id).then((res) => {
          if (res?.data) {
            reset({
              name: res.data.name || '',
              alias: res.data.alias || '',
              sequenceNo: res.data.sequenceNo || '',
              icon: res.data.icon || '',
              url: res.data.url || '',
  ...
  const onSubmit: SubmitHandler<EditFormValues> = async (data) => {
    ...
    try {
      const response = await updated('/api/v1/menu', initValues.id, payload);
          ...
          <Form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-7">
            <div>
              ...
              <Input
                ...
                className={`mt-1 block px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white md:w-[200px] ${
