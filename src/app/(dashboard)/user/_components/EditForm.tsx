...
import { EditFormValues, User } from '../_interfaces/User';
...
const EditForm: React.FC<EditModalsProps<User>> = ({
  title,
  initValues,
  isOpen,
  onClose,
  ...
  useEffect(() => {
    if (isOpen && initValues) {
      ...
      Promise.all([
        getById('/api/v1/user', initValues.id).then((res) => {
          if (res?.data) {
            reset({
            ...
            if (res.data.avatarFile) {
              setAvatarPreview(
                `${process.env.NEXT_PUBLIC_CDN_URL}/avatar/${res.data.avatarFile}`,
              );
            } else {
  ...
  const onSubmit: SubmitHandler<EditFormValues> = async (data) => {
    ...
    try {
      const response = await updated('/api/v1/user', initValues.id, payload);
