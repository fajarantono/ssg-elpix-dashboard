...
import { EditFormValues, Role } from '../_interfaces/Role';
...
const schema = yup.object().shape({
  ...
  description: yup.string().required('Is Root is required'),
  isRoot: yup.boolean().required('Is Root is required'),
  isActive: yup.boolean().required('Is Active is required'),
});

const EditForm: React.FC<EditModalsProps<Role>> = ({
  title,
  initValues,
  isOpen,
  onClose,
  ...
  useEffect(() => {
    if (isOpen && initValues) {
      setIsLoading(true);
      getById('/api/v1/role', initValues.id)
        .then((res) => {
          if (res?.data) {
            reset({
              name: res.data.name || '',
  ...
  const onSubmit: SubmitHandler<EditFormValues> = async (data) => {
    ...
    try {
      const res = await updated('/api/v1/role', initValues.id, payload);
