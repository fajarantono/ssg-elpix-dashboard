...
import { AddFormValues } from '../_interfaces/Role';
...
const AddForm: React.FC<AddModalsProps> = ({ title, isOpen, onClose, refresh }) => {
  ...
  const onSubmit: SubmitHandler<AddFormValues> = async (data) => {
    ...
    try {
      const response = await created('/api/v1/role', payload);
